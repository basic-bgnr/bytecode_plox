...
class ScopeEntity:
    def __init__(self, name, scope_depth, index=None):
        ...
        self.scope_depth = (
            scope_depth  # scope depth is for popping the value when scope is ended
        )
...
class Compiler:
    def __init__(self):
        ...
        self.scope_entities = (
            []
        )  # position of ScopeEntity is the position of the variable in the stack, enumerate(scapepositoin) => (stack_index, scope_entitiy)
    ...
    def visitLiteralExpression(self, literal_expression):
        if literal_expression.expr.tipe is TokenType.NUMBER:
            number = MasterData(
                tipe=LanguageTypes.NUMBER, value=literal_expression.value
            )
            self.pushConstant(number, at_line=literal_expression.expr.line)

        elif literal_expression.expr.tipe is TokenType.TRUE:
            self.pushConstant(
                LanguageConstants.TRUE, at_line=literal_expression.expr.line
            )

        elif literal_expression.expr.tipe is TokenType.FALSE:
            self.pushConstant(
                LanguageConstants.FALSE, at_line=literal_expression.expr.line
            )

        elif literal_expression.expr.tipe is TokenType.NIL:
            self.pushConstant(
                LanguageConstants.NIL, at_line=literal_expression.expr.line
            )

        elif literal_expression.expr.tipe is TokenType.STRING:
            string = MasterData(
                tipe=LanguageTypes.STRING, value=literal_expression.value
            )
        ...
        ##todo: identifier_token implementation
            ...
            local_variable_index = self.resolveLocalVariable(
                literal_expression.expr.literal
            )
            if local_variable_index is not None:
                self.emitCodes(
                    OpCode.OP_LOAD_LOCAL,
                    local_variable_index,
                    at_line=literal_expression.expr.line,
                )
            else:
                variable_name = MasterData(
                    tipe=LanguageTypes.STRING, value=literal_expression.expr.literal
                )
    ...
    def visitExpressionStatement(self, expression_statement):
        ...
        if expression_statement.expr:
            ...
            self.emitCode(
                OpCode.OP_POP, at_line=line_num
            )  # extra pop due to function expression statement is due to this.
            return line_num
    ...
    def endScope(self, line_num, is_function_statement=False):
        ...
        while (
            not self.isScopeEntitiesEmpty()
            and self.getLastScopeEntityDepth() > self.getScopeDepth()
        ):
    ...
    def visitAssignmentStatement(self, assignment_statement):
        if isinstance(assignment_statement.lvalue, GetExpression):
            ...
            # compile the lhs
            line_num = self.pushConstant(
                constant=MasterData(
                    tipe=LanguageTypes.STRING,
                    value=assignment_statement.lvalue.prop_or_method.literal,
                ),
                at_line=line_num,
            )
    ...
    def visitReassignmentStatement(self, reassignment_statement):
        if isinstance(reassignment_statement.lvalue, GetExpression):
            ...
            line_num = self.pushConstant(
                constant=MasterData(
                    tipe=LanguageTypes.STRING,
                    value=reassignment_statement.lvalue.prop_or_method.literal,
                ),
                at_line=line_num,
            )
    ...
    def addGlobalReassignment(self, reassignment_statement):
        line_num = self.compile(reassignment_statement.rvalue)
        lvalue_name = MasterData(
            tipe=LanguageTypes.STRING, value=reassignment_statement.lvalue.expr.literal
        )

        global_variable_index = self.makeConstant(lvalue_name)
        self.emitCodes(
            OpCode.OP_REDEFINE_GLOBAL, global_variable_index, at_line=line_num
        )
        return line_num

    def addGlobalAssignment(self, assignment_statement):
        line_num = self.compile(
            assignment_statement.rvalue
        )  # this pushes rvalue in the program stack
        lvalue_name = MasterData(
            tipe=LanguageTypes.STRING, value=assignment_statement.lvalue.expr.literal
        )

        global_variable_index = self.makeConstant(
            lvalue_name
        )  # make the lvalue into MasterData and store it in the constant table
    ...
    def addLocalAssignment(self, assignment_statement):
        ...
        line_num = self.compile(
            assignment_statement.rvalue
        )  # this just put the rvalue in the stack
    ...
    def visitReturnStatement(self, return_statement):
        if return_statement.expr is not None:
            ...
            line_num = 100
            # empty return statement won't produce any error, so putting random value
    ...
    def visitContinueStatement(self, continue_statement):
        ...
        self.emitByte(
            byte=goto_entity.instruction_pointer_location, at_line=line_num
        )  # this byte will get modified later on
    ...
    def visitFunctionStatement(self, function_statement):
        ...
        if not self.isInsideClass():  # only global function are callable by names
            function_name = (
                function_statement.function_identifier_expression.expr.literal
            )
            params_length = len(function_statement.params_list)

            function_constant = MasterData(
                tipe=LanguageTypes.FUNCTION,
                value=FunctionObject(name=function_name, arity=params_length, ip=ip),
            )
            function_index = self.makeConstant(constant=function_constant)

            # this is for use in the vm for indexing function table
            function_name_constant = MasterData(
                tipe=LanguageTypes.STRING, value=function_name
            )
            function_name_constant_index = self.makeConstant(
        ...
        self.addScopeEntity(
            ScopeEntity(name="@ebp", scope_depth=self.getScopeDepth(), index=-1),
            default=False,
        )
        self.addScopeEntity(
            ScopeEntity(name="@retptr", scope_depth=self.getScopeDepth(), index=-2),
            default=False,
        )
        ...
        for index, param in zip(indices, reversed(function_statement.params_list)):
            self.addScopeEntity(
                ScopeEntity(
                    name=param.expr.literal,
                    scope_depth=self.getScopeDepth(),
                    index=index,
                ),
                default=False,
            )
        ...
        self.endScope(
            line_num, is_function_statement=True
        )  # the second arguemnt causes to not emit `pop` code
    ...
    def visitCallableExpression(self, function_expression):
        ...
        index_to_modify_ret_ptr = (
            self.getNextIPLocation()
        )  # this is to modify the next byte
        self.emitByte(
            byte=None, at_line=line_num
        )  # we fill it with none, because now we don't know where the return pointer should point to

        self.pushConstant(
            MasterData(tipe=LanguageTypes.NUMBER, value=len(function_expression.args)),
            at_line=line_num,
        )  # pus the current no of argument supplied to the function, this used for checking if correct no. of arugment is supplied during runtime
        self.compile(
            function_expression.caller_expr
        )  # load the function object in the stack
        ...
        ret_pointer_index = self.makeConstant(
            constant=MasterData(
                tipe=LanguageTypes.NUMBER, value=self.getNextIPLocation()
            )
        )  # this is to modify the next byte), at_line=line_num)
        # print('ret_pointer_index ', ret_pointer_index)
        self.modifyByteAt(
            byte=ret_pointer_index, index=index_to_modify_ret_ptr
        )  # this set the value of the byte previously set o None, as we now know where the function is going to end
    ...
    def addScopeEntity(self, scope_entity, default=True):
        if default:
            scope_entity.index = (
                self.getScopeEntitiesSize() - self.getScopeEntittiesDatum()
            )  # this makes the referencing relative
    ...
    def visitIfStatement(self, if_statement):
        ...
        )  # this pushes the value in the stack

        self.emitCode(OpCode.OP_JMP_IF_FALSE, line_num)
        self.emitByte(
            None, line_num
        )  # just some random byte, we'll fill it up with true value once the conditional statements are compiled

        branch_if_start_instruction_pointer = (
            self.getInstructionPointerSize() - 1
        )  # this points to the above byte

        if_line_num = self.compile(
            if_statement.if_block_statement
        )  # push the branch code in the code stack

        if if_statement.else_block_statement is None:
            branch_if_end_instruction_pointer = (
                self.getInstructionPointerSize() - 1
            )  # points to above byte
            offset_if = (
                branch_if_end_instruction_pointer - branch_if_start_instruction_pointer
            )  # the number of additional line pushed in the codes stack

            self.modifyByteAt(
                byte=offset_if, index=branch_if_start_instruction_pointer
            )  # update the None byte to offset

            return if_line_num

        else:
            self.emitCode(
                OpCode.OP_JMP, at_line=if_line_num
            )  # we put this jump statement so that if the condition is true, the code executes as usual and jump unconditionally after reaching the end of true block, it needs to jump by amount of size of else block.
            self.emitByte(
                None, at_line=if_line_num
            )  # just some random byte, we'll fill it up with true value one the
            ...
            offset_else = (
                branch_else_end_instruction_pointer - branch_if_end_instruction_pointer
            )

            self.modifyByteAt(
                byte=offset_else, index=branch_if_end_instruction_pointer
            )  # push offset to the end of true statement's end

            return else_line_num

    def visitWhileStatement(self, while_statement):
        self.pushBreakEntity(
            break_entity=LoopModifierEntity(
                parent_loop_scope_depth=self.getScopeDepth(),
                instruction_pointer_location=None,
            )
        )  # we don't know the ip location now
        self.pushContinueEntity(
            continue_entity=LoopModifierEntity(
                parent_loop_scope_depth=self.getScopeDepth(),
                instruction_pointer_location=None,
            )
        )  # we don't know the ip location now

        top_label_instruction_pointer = (
            self.getInstructionPointerSize()
        )  # this points to below expression

        line_num = self.compile(
            while_statement.expression
        )  # put the condition expression in the stack

        self.emitCodes(
            OpCode.OP_JMP_IF_FALSE, None, at_line=line_num
        )  # put random byte at the jump location
        branch_if_start_instruction_pointer = (
            self.getInstructionPointerSize() - 1
        )  # this indexes the above Null byte
        ...
        branch_if_end_instruction_pointer = (
            self.getInstructionPointerSize() + 1 + 1
        )  # this points to the following op_jmp, but we need to add one since loop ofset variable is also in the code stack and an additional 1 so that the ip_counter point next to it.

        loop_offset = (
            top_label_instruction_pointer - branch_if_end_instruction_pointer
        )  # this is negative, as we need to go upward
        self.emitCodes(
            OpCode.OP_JMP, loop_offset, at_line=loop_line_num
        )  # this loops to the start o the top_level_counter

        false_condition_instruction_pointer = (
            self.getInstructionPointerSize() - 1
        )  # this points to the instruction after the above OP_JMP

        self.modifyBreakInstructionPointer()  # starts break from here

        false_offset = (
            false_condition_instruction_pointer - branch_if_start_instruction_pointer
        )
    ...
    def modifyContinueInstructionPointer(self):
        ...
        if index := continue_entity.instruction_pointer_location:
            modified_ip = (
                self.getInstructionPointerSize() - 1 - index
            )  # this relative position points upwards
    ...
    def modifyBreakInstructionPointer(self):
        ...
        if index := break_entity.instruction_pointer_location:
            modified_ip = (
                self.getInstructionPointerSize() - 1 - index
            )  # this relative position points downwards
            self.modifyByteAt(byte=modified_ip, index=index)
    ...
    def visitGetExpression(self, get_expression):
        ...
        self.pushConstant(
            MasterData(tipe=LanguageTypes.STRING, value=prop_or_method_token.literal),
            at_line=prop_or_method_token.line,
        )
    ...
    def visitClassStatement(self, class_statement):
        ...
        class_constant = MasterData(
            tipe=LanguageTypes.CLASS, value=ClassObj(name=class_name)
        )
