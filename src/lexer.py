...
class Scanner:
    ...
    def scanToken(self):
        ...
        elif (lexeme in [TokenType.CARRIAGE_RETURN.value, 
                         TokenType.SPACE.value,
                         TokenType.VERTICAL_TAB.value ,
                         TokenType.HORIZONTAL_TAB.value]): #ignore any of the case of return feed, space and tab
                ...
                if (next_value != TokenType.STRING.value): #check for closing " value
                    if (next_value == TokenType.NEW_LINE.value): # allows multiline string but the edge condition is that we need to implicitly add line
                        ...
                        if next_value == 'n':
                            ...
                            raise Exception(f"no matching escape sequence at line {self.line}")
                    else:
                        accumulated_string += next_value
                        self.advance()
