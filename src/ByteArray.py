...
class Chunk:
    def __init__(self):
        ...
        self.constants = [
            LanguageConstants.TRUE,
            LanguageConstants.FALSE,
            LanguageConstants.NIL,
        ]  # container for literal value
        self._constant_map = defaultdict(
            lambda: None,
            {constant: index for index, constant in enumerate(self.constants)},
        )  # key: constant, value: index,=> self.constants[self._constant_map[key]]
    ...
    def getCodesLength(self):
        return len(
            self.codes
        )  # returns addressable index(this doesn't need subtraction, can be directly indexed)
