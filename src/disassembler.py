...
class Disassembler:
    ...
    def pretty_print(self):
        initializing_codes = self.formatOpCodes(
            header="INITIALIZING_CODES", chunk=self.initializing_codes.codes
        )

        index_of_halt = Disassembler.findIndexOfHalt(self.chunk.codes) + 1

        program_code = self.formatOpCodes(
            header="PROGRAM_CODE",
            chunk=self.chunk.codes[0:index_of_halt],
            origin_line_number=len(self.initializing_codes.codes),
        )
        function_object_code = self.formatOpCodes(
            header="FUNCTION_OBJECT_CODE",
            chunk=self.chunk.codes[index_of_halt:],
            origin_line_number=len(self.initializing_codes.codes) + index_of_halt,
        )

        constant_pool = Disassembler.stringifyConstantPool(
            header="CONSTANT POOL", constant_pool=self.chunk.constants
        )

        return "\n".join(
            constant_pool + initializing_codes + program_code + function_object_code
        )
    ...
    def formatOpCodes(self, header, chunk, origin_line_number=0):
        ...
        op_codes_followed_by_bytes = [
            OpCode.OP_LOAD_CONSTANT,
        ...
        for line_number, op_code in line_code_pair:
            if op_code in op_codes_followed_by_bytes:
                ...
                value = (
                    self.constantAt(index=next_byte)
                    if op_code in op_codes_global
                    else "[...]"
                )

                output_string.append(
                    Disassembler.stringifyOpCode(
                ...
                output_string.append(
                    Disassembler.stringifyOpCode(
                        origin_line_number + line_number, op_code
                    )
                )
